...
You stand at `~/confit-demo` with shell plus kitty configs in place. This chapter names each file shape those configs use.
...
Drift compares key by key. One changed value reports its dotted key with old plus new. A disk file that no longer parses falls back to a text hunk.

## Text

The demo kitty config uses this shape for `kitty.conf` plus the desktop list. Raw content, byte for byte. Templates render first, then the result lands verbatim:
...
The demo kitty config uses this shape for its binary plus desktop links. A symlink from path to target:

```lua
confit.document.link(path, target)
```

Drift compares the target string. A moved link reports old target plus new target.
