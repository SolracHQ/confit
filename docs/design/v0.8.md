...
streams each ref through gzip into its archive entry, the gzip landing in a
temp file first since archive headers need the size upfront. The on-disk
...
flowchart LR
    ...
    built --> drift[Drift streams destination files]
...
The previous state loads as manifest only. Drift, summary, counts, and the
rewrite set already run on hashes, and drift gains a streaming file hash:
destination bytes stream in chunks against the recorded sha, tree members the
same per file, with no pool access and no bundle bytes. The disk short-circuit
and the unmanaged skip stay ahead of all of it, so steady runs hash alone and
stop there.
