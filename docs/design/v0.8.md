...
the plan's bytes landing where the plan named them. The exact mechanism stays
...
fresh regular file in its place. The report names the swap instead of a bare
`wrote`:

```text
~/.config/starship.toml: link replaced with file
```

Drift already reads through the link for its content diff, so the preview shows
...
come back from the declared content on the next apply, same as today. The
preview keeps unmanaged documents off the green already-in-place line, that line
promises matching bytes. It gets its own marker instead:
