...
local utils = confit.utils
local is_array = utils.is_array
local holds_cycle = utils.holds_cycle
...
local function merge(base, overlay, opts)
	...
	if holds_cycle(base) then
		confit.plugin.helpers.error("merge: base holds a recursive table")
	end
	if holds_cycle(overlay) then
		confit.plugin.helpers.error("merge: overlay holds a recursive table")
	end
