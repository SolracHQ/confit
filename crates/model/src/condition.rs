...
pub enum Condition {
    ...
    Exists {
        /// Holds the route under testing.
        route: Route,
    },
    /// Holds a changed document assertion.
    ///
    /// True while `route` previews a write, drift, or absence.
    /// Hooks only.
    Changed {
        /// Holds the document route under testing.
        route: Route,
    },
...
impl Condition {
    ...
    ///
    pub fn collect_changed(&self, out: &mut Vec<Route>) {
        match self {
            Self::Changed { route: dest } => out.push(dest.clone()),
