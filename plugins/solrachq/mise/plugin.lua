...
-- opts table carries `lane` through; the guard always applies.
local function collector(config, binary)
  ...
  local function opts_with_guard(opts)
    ...
    if opts ~= nil then
      ...
      if opts.lane ~= nil then
        merged.lane = opts.lane
      end
    end
    return merged
  end
...
-- an optional opts table carries `lane` through. Callers attach it with
-- `config:add_document`.
local function activate(opts)
  if opts ~= nil then
    if type(opts) ~= "table" then
      confit.plugin.helpers.error("mise: activate opts must be a table")
    end
    if opts.lane ~= nil and type(opts.lane) ~= "string" then
      confit.plugin.helpers.error("mise: field 'lane' must be a string")
    end
  end
  return confit.document.rc.eval({ "mise", "activate", confit.shell.SHELL }, opts)
end
